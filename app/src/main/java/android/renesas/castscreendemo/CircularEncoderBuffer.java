...
public class CircularEncoderBuffer {
    ...
    public int getFirstIndex() {
        ...
        while (index != mMetaHead) {
            Log.d(TAG, "getFirstIndex() metaLen="+metaLen+" index="+index+" flags="+mPacketFlags[index]);

            if ((mPacketFlags[index] & BUFFER_FLAG_SYNC_FRAME) != 0) {
                Log.w(TAG, "getFirstIndex() BUFFER_FLAG_SYNC_FRAME");
                break;
            }
            //Log.w(TAG, "getFirstIndex: loop index="+index );
            index = (index + 1) % metaLen;
        }

        if (index == mMetaHead) {
            Log.e(TAG, "HEY: could not find sync frame in buffer");
            index = -1;
        }
        Log.d(TAG, "getFirstIndex() OK");
        return index;
    }
